...
func (uh *UserHandler) Register(w http.ResponseWriter, r *http.Request) {
	...
	r.Body.Close()
	if err != nil {
		uh.Logger.Errorw("can`t read body of request", err)
	...
	if err != nil {
		uh.Logger.Errorw("can`t unmarshal register form", err)
		http.Error(w, "bad reg data", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		uh.Logger.Errorw("can`t validate register form", err)
		http.Error(w, "bad reg data", http.StatusBadRequest)
		return
	}

	if user.Role != "user" {
		uh.Logger.Errorw("can`t register not user role", err)
		http.Error(w, "bad reg data", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		uh.Logger.Errorw("can`t create user", err)
	...
	if err != nil {
		uh.Logger.Errorw("can`t create session", err)
	...
	if err != nil {
		uh.Logger.Errorw("can`t marshal session token", err)
		http.Error(w, "can`t make session", http.StatusInternalServerError)
		return
	}
	...
	_, err = w.Write(resp)
	if err != nil {
		uh.Logger.Errorw("can`t write response", err)
...
func (uh *UserHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		uh.Logger.Errorw("can`t get user", err)
